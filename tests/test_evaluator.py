...
def test_evaluator_simple_case():
    ...
    evaluator = OldEvaluator(true, pred, tags=["LOC", "PER"])
...
def test_evaluator_simple_case_filtered_tags():
    ...
    evaluator = OldEvaluator(true, pred, tags=["PER", "LOC"])
    results, _, _, _ = evaluator.evaluate()
    expected = {
        "strict": {
            "correct": 3,
...
def test_evaluator_extra_classes():
    ...
    evaluator = OldEvaluator(true, pred, tags=["ORG", "FOO"])
    results, _, _, _ = evaluator.evaluate()
    expected = {
        "strict": {
            "correct": 0,
...
def test_evaluator_no_entities_in_prediction():
    ...
    evaluator = OldEvaluator(true, pred, tags=["PER"])
    results, _, _, _ = evaluator.evaluate()
    expected = {
        "strict": {
            "correct": 0,
...
def test_evaluator_compare_results_and_results_agg_1():
    ...
    evaluator = OldEvaluator(true, pred, tags=["PER", "ORG", "MISC"])
    results, results_agg, _, _ = evaluator.evaluate()
    expected = {
        "strict": {
            "correct": 2,
...
def sample_data():
    true = [
        ...
        [
            Entity(label="PER", start=0, end=0),
            Entity(label="ORG", start=2, end=2)
        ]
    ]
    ...
    pred = [
        ...
        [
            Entity(label="PER", start=0, end=0),  # Correct
            Entity(label="LOC", start=2, end=2)   # Wrong type
        ]
    ]
